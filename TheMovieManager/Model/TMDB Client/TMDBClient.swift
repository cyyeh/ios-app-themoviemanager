...
class TMDBClient {
    ...
    static let apiKey = "5017052e4fc58ce19a8bdf2651078a75"
    ...
    enum Endpoints {
        ...
        var stringValue: String {
            ...
            case .searchMovie(let query): return Endpoints.base + "/search/movie" + Endpoints.apiKeyParam + "&query=\(query.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? "")"
    ...
    class func searchMovie(query: String, completion: @escaping ([Movie], Error?) -> Void) -> URLSessionTask {
        let task = taskForGETRequest(url: Endpoints.searchMovie(query).url, response: MovieResults.self) { (response, error) in
            if let response = response {
                completion(response.results, nil)
            } else {
                completion([], error)
